...
def get_subscriptions(broadcast_data: dict, subscriptions: list[dict[str, str]]) -> Iterable[Subscription]:
    subscription_target_folders: dict[str, Path] = {
        subscription["title"]: Path(subscription["targetFolder"]) for subscription in subscriptions
    }
    broadcasts_per_day = (day["broadcasts"] for day in broadcast_data)
    broadcasts = itertools.chain.from_iterable(broadcasts_per_day)
    subscribed_broadcasts = (b for b in broadcasts if b["title"] in subscription_target_folders)
...
def get_download_tasks(
    base_url: str, stream_base_url: str, subscriptions: list[dict[str, str]]
) -> Iterable[DownloadTask]:
    broadcasts = get_broadcasts(base_url)
    tasks = (get_streams(base_url, task, stream_base_url) for task in get_subscriptions(broadcasts, subscriptions))
